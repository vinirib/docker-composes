...
function ensure_role {
    ...
    local elasticsearch_host="${ELASTICSEARCH_HOST:-elasticsearch}"
    local args=('-s' '-D-' '-m15' '-w' '%{http_code}'
    ...
    echo "Sending the following JSON to Elasticsearch for role creation: $body"
    ...
    if ((result)); then
        echo -e "\nFailed to create/update role. Response from Elasticsearch:\n${output::-3}\n"
    else
